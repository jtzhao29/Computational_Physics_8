...
def run_simulation(L, T, n_eq=600000, n_meas=1600000):
    ...
    E_list = np.empty(n_meas)
    M_list = np.empty(n_meas)
    for i in range(n_meas):
        lattice = metropolis_step(lattice, beta)
        E_list[i] = calculate_energy(lattice)
        M_list[i] = calculate_magnetization(lattice)
    return E_list, M_list
def plot_observables(T_list, L_list):
    os.makedirs("images", exist_ok=True)
    
    # Cache to store the results of run_simulation for each (L, T)
    simulation_results = {}

    # Precompute the simulation results for all L and T combinations
    for L in L_list:
        for T in T_list:
            E, M = run_simulation(L, T)
            print(L,T)
            simulation_results[(L, T)] = (E, M)
